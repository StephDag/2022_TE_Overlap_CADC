...
marine_terrestial_raster <- function(point,buffer_ter,buffer_marine,biodiv) { # point = sf dataframe, buffer.search = in meters, buffer.impact in meters
  ...
  # Create a search buffer around the point of 150km
  ...
  # Get the index of the nearest cell to the point in the 150km buffer
  ...
  # from this point, draw a 20km marine buffer and estimate the mean number of species (or other)
  ...
  # return as a lits
  data.frame(point = point, mean.count=mean.count,marine.pt=xy.index,distance=as.numeric(index[3]), mean.count.grav = mean.count.grav)
