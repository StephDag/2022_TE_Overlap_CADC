...
#point <-sf_obj
#point <- sf_obj.chunks
buffer_ter <- 120000 # 150km terrestrial raster
...
# Loop through sf_obj.chunks using foreach
foreach(i = 1:length(sf_obj.chunks), .packages = c("raster", "sp")) %dopar% {
  print(paste("i=",i))
  point <- sf_obj.chunks[[i]]
  results.sp.count.risk.temp <- foreach(j = split(point, seq(nrow(point))), .combine = rbind.data.frame) %dopar% {
    mcf(marine_terrestial_raster)(j, buffer_ter = buffer_ter, buffer_marine = buffer_marine, biodiv = biodiv)
  }
...
results.sp.count.risk.df.full <- results.sp.count.risk.df.full %>%
